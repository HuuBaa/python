...
class WeatherSaxHandler(object):
    ...
    def start_ele(self,name,attrs):
        ...
        if self.currentData=='yweather:condition':
            self.date=int(attrs['date'][5:7])
        if self.currentData=='yweather:location':
            self.weather['city']=attrs['city']
            self.weather['country']=attrs['country']
        if self.currentData=='yweather:forecast' and self.date== int(attrs['date'][:2]):
            
            self.weather['today']={}
            self.weather['today']['text']=attrs['text']
            self.weather['today']['low']=int(attrs['low'])
            self.weather['today']['high']=int(attrs['high'])            
        if self.currentData=='yweather:forecast' and self.date + 1== int(attrs['date'][:2]):
